...
namespace vcpkg {
	fs::path toolset_env_path() {
		ComPtr<ISetupConfiguration> config = nullptr;
		HRESULT hr = CoCreateInstance(__uuidof(SetupConfiguration), nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&config));
		if (hr != S_OK || config == nullptr) {
			Checks::exit_with_message("Could not find any Visual Studio Instances");
		}
		
		ComPtr<ISetupConfiguration2> config2 = nullptr;
		hr = config.As(&config2);
		if (hr != S_OK || config2 == nullptr) {
			Checks::exit_with_message("Unsupported version of VS 15 may be installed");
		}
		ComPtr<IEnumSetupInstances> enumInstances;
		
		hr = config2->EnumAllInstances(&enumInstances);
		if (hr != S_OK || enumInstances == nullptr) {
			Checks::exit_with_message("Could not enumerate visual studio instances");
		}
		ComPtr<ISetupInstance> instance;
		ComPtr<ISetupInstance2> latestInstance = nullptr;
		while (enumInstances->Next(1, &instance, nullptr) == S_OK) {
			ComPtr<ISetupInstance2> instance2;
			hr = instance.As(&instance2);
			if (hr != S_OK) {
				Checks::exit_with_message("Could not query instance for ISetupInstance2");
			}
			BSTR ver;
			BSTR highestVer;
			if (!latestInstance) latestInstance = instance2;
			hr = instance2->GetInstallationVersion(&ver);
			if (hr != S_OK) Checks::exit_with_message("Could not get install version");
			hr = latestInstance->GetInstallationVersion(&highestVer);
			if (hr != S_OK) Checks::exit_with_message("Could not get highest instance version");
			std::wstring verstr(ver);
			std::wstring lateststr(highestVer);
			if (ver > highestVer) {
				latestInstance = instance2;			
			}

		}
		if (!latestInstance) {
			Checks::exit_with_message("Could not get any instances");
		}
		BSTR install_path;
		hr = latestInstance->GetInstallationPath(&install_path);
		if (hr != S_OK) Checks::exit_with_message("Could not get installation path");
		std::wstring_convert<std::codecvt_utf8<wchar_t>> cvt;
		std::string install_pathstr = cvt.to_bytes(std::wstring(install_path));
		fs::path cmd_path = fs::path(install_pathstr) / fs::path("Common7") / fs::path("Tools") / fs::path("VsDevCmd.bat");
		Checks::check_exit(fs::exists(cmd_path), "VS Command path %s does not exist", cmd_path.u8string());
		return cmd_path;
	}
}